...
def upload_file(df, origname=None, outdir=".", secret_token=secrets.token_hex(8)):
    ...
    tb.write(os.path.join(outdir, filename), delimiter=",", format="ascii.ecsv", overwrite=True)
...
def PPPrunStart( uS, weight_para):
    ...
    def target_collision(sample,sep=2/3600.):
    ...
    def netflowRun(sample):
        ...
        point_t=Table(np.array(point_list,dtype=object),names=['ppc_code','group_id','ppc_ra','ppc_dec',\
                                                               'ppc_pa','ppc_priority','tel_fiber_usage_frac',\
                                                               'allocated_targets','ppc_resolution'],\
    ...
    def complete_ppc(sample,point_l):
        ...
        point_l_pri=point_l[point_l.argsort(keys='ppc_priority')] #sort ppc by its total priority == sum(weights of the assigned targets in ppc)
        ...
        for ppc in point_l_pri:
            ...
            comT_t=[len(comp_s)] + [sum(sample['priority'].data[comp_s]==ll) for ll in sub_l]
    ...
    if len(uS_L) > 0 and len(uS_M) > 0:
        ...
        return  uS_L2, cR_L, cR_L_, sub_l, obj_allo_L_fin, uS_M2, cR_M, cR_M_, sub_m, obj_allo_M_fin
...
def ppp_result(cR_l, sub_l, obj_allo_l, uS_L2, cR_m, sub_m, obj_allo_m, uS_M2):
    ...
    def ppp_plotFig(RESmode,cR,sub,obj_allo,uS):
        ...
        nppc=pn.widgets.IntSlider(name='You can modify the number of pointings for the '+RESmode+' resolution',\
                                  value=len(cR), step=1, start=1, end=len(cR),bar_color='gray',width=500)
        ...
        def plot_ppc(nppc_fin):
            ...
            def PFS_FoV_plot(ppc_ra,ppc_dec,PA):   
                ...
                ppc_tot_plot = [hv.Area(ii).opts(color = 'gray',alpha = 0.2,line_width = 0) for ii in ppc_coord]
                
                pd_ppc = pd.DataFrame({'RA':ppc_ra, 'DEC':ppc_dec, 'PA':PA})
                p1 = pd_ppc.hvplot.scatter(x = 'RA', y = 'DEC', by = 'PA', \
                                        title='Distribution of targets & PPC',color='gray',marker='s',s=40, legend=False)
                
                return p1*hv.Overlay(ppc_tot_plot)
            
            p_ppc = PFS_FoV_plot(obj_allo1['ppc_ra'][:nppc_fin], obj_allo1['ppc_dec'][:nppc_fin], obj_allo1['ppc_pa'][:nppc_fin])

            p_tgt = uS_.hvplot.scatter(x = 'ra', y = 'dec', by = 'priority', color=['r']+colors, marker='o', s=20, legend=True)
        ...
        def plot_CR(nppc_fin):
            ...
            p1=cR__.hvplot.line('PPC_id',name[:-1],value_label='Completion rate (%)',title=f'{RESmode:s}-resolution mode',\
                                color=['k', 'r']+colors,line_width=[3, 2] + [1]*(len(sub)-1),\
                                line_dash=['solid']*2+['dashed']*(len(sub)-1))
            p2=hv.Rectangles([(30,88, 95, 100)]).opts(color='orange',line_width=0,alpha=0.2)
            p3=hv.Rectangles([(20,43,130, 93)]).opts(color='dodgerblue',line_width=0,alpha=0.2)
            p4=hv.VLine(nppc_fin).opts(color='gray',line_dash='dashed',line_width=5)

            return (p1*p2*p3*p4).opts(xlim=(0,len(obj_allo)+1),ylim=(0,105),show_grid=True)

        def plot_FE(nppc_fin):
            mean_FE=np.mean(obj_allo2['Fiber usage fraction (%)'][:nppc_fin])
            p1=obj_allo2.hvplot.bar('PPC_id','Fiber usage fraction (%)',title=f'{RESmode:s}-resolution mode',\
                               rot=90, width=1,color='tomato',alpha=0.5,line_width=0)
            p2=(hv.HLine(mean_FE).opts(color='red',line_width=3)) * (hv.Text(int(len(cR)*.85), \
                                                                             mean_FE*1.5, '{:.2f}%'.format(mean_FE)).opts(color='red'))
            p3=hv.VLine(nppc_fin).opts(color='gray',line_dash='dashed',line_width=5)
        
            return (p1*p2 *p3).opts(fontsize={'xticks': '0pt'},xlim=(0,len(obj_allo)+1),\
                                    ylim=(0,max(obj_allo2['Fiber usage fraction (%)'][:nppc_fin])+1))
        
        def ppp_res_tab1(nppc_fin):
            hour_tot = nppc_fin * 15./60. # hour
            Fhour_tot = sum([len(tt) for tt in obj_allo1[:nppc_fin]['allocated_targets']]) * 15./60. # fiber_count*hour
            Ttot_best, Ttot_worst=overheads(nppc_fin)
            fib_eff_mean = np.mean(obj_allo1['Fiber usage fraction (%)'][:nppc_fin])
            fib_eff_small = sum(obj_allo1['Fiber usage fraction (%)'][:nppc_fin]<30) / nppc_fin * 100.

            cR1 = pd.DataFrame(dict(zip(name[:-1], cR[nppc_fin-1])), index=[0],)
        ...
        p_result_fig = pn.Row(pn.Column(pn.bind(plot_CR,nppc),width=700,height=300,), pn.Column(pn.bind(plot_FE,nppc),width=500,height=285,), \
                              pn.Column(pn.bind(plot_ppc,nppc),width=600,height=300,), )
    ...
    if len(cR_l) > 0 and len(cR_m) ==0:
        nppc_l, p_result_fig_l, p_result_tab_l, p_result_ppc_l = ppp_plotFig('low', cR_l, sub_l, obj_allo_l, uS_L2)
        
        return 'low', nppc_l, p_result_fig_l, p_result_ppc_l, p_result_tab_l
    
    elif len(cR_m) > 0 and len(cR_l) == 0:
        nppc_m, p_result_fig_m, p_result_tab_m, p_result_ppc_m = ppp_plotFig('medium', cR_m, sub_m, obj_allo_m, uS_M2)
        ...
        return 'low & medium', nppc_fin, p_result_fig_fin, p_result_ppc_fin, p_result_tab#'''
...
def visibility_checker(uS,semester):
    ...
    for i_t in range(len(RA)):
        target = StaticTarget(name = ob_code[i_t], ra = RA[i_t], dec = DEC[i_t], equinox=2000.0)
        total_time = exptime[i_t] # SEC
        
        t_obs_ok = 0
        
        for dd in range(len(daterange)-1):
            night_begin = parser.parse(daterange[dd].strftime("%Y-%m-%d") + " 18:30:00").replace(tzinfo=tz_HST)
            night_end = parser.parse(daterange[dd+1].strftime("%Y-%m-%d") + " 05:30:00").replace(tzinfo=tz_HST)
            observer.set_date(night_begin)
            
            obs_ok, t_start, t_stop = observer.observable(target, night_begin, night_end,
                                                          min_el, max_el, total_time,
